...
function getNumberFrom() {
  ...
  return isNaN(numberFromUserInput) || numberFromUserInput < MIN_VALUE_FROM
    ? DEFAULT_FROM
    : numberFromUserInput
}
...
export function newNumber() {
  ...
  const koreanNumberSystem =
    document.querySelector('#form-field-system').value === 'korean'
  const showWritten = getShowNumberWrittenOption()
  try {
    let writtenNumber = koreanNumberSystem
      ? getNumberWrittenKorean(randomNumber)
      : getNumberWrittenChinese(randomNumber)
    showWritten
      ? setDisplays(writtenNumber, randomNumber)
      : setDisplays(randomNumber, writtenNumber)
  } catch (e) {
...
export function newTime() {
  ...
  getShowNumberWrittenOption()
    ? setDisplays(written, digits)
    : setDisplays(digits, written)
}
...
export function getNumberWrittenKorean(number, useAbbreviated = false) {
  ...
  var output = ''
  output =
    numbersWrittenKorean[0][useAbbreviated ? 'abbreviated' : 'normal'][
      numberAsStringReversed.charAt(0)
    ] + output
...
export function getNumberWrittenChinese(number) {
  ...
  output =
    (currentChar > 0 ? numbersWrittenChinese[0][currentChar] : '') + output
...
export function newWeekDay() {
  ...
  getShowWeekdayEnglishOption()
    ? setDisplays(randomWeekDayEnglish, weekDays[randomWeekDayEnglish])
    : setDisplays(weekDays[randomWeekDayEnglish], randomWeekDayEnglish)
}
